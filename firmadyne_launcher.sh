...
if [ "$#" -ge 1 ]; then
    ...
    echo "Example: ./firmadyne_launcher.sh Router-firmware.zip"
    exit 1
fi
...
# temporary modify firmadyne's code in order to exclude the db
sed -e "/psql -d firmware/ s/^#*/#/" -i scripts/getArch.sh
...
# cleanup
echo 'Cleaning up...'
sed -i "/^#.* psql -d /s/^#//" scripts/getArch.sh
rm images/${ID}.tar.gz
